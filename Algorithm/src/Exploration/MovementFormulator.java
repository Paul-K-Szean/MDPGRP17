...
public class MovementFormulator {
    ...
    public void predictAndSendCalibrationReminder(Robot _robot, RobotMovement action) {

        Robot robotSimulator = new Robot(new GridVector(_robot.position().x(), _robot.position().y()), _robot.orientation());
        robotSimulator.execute(action);

        if (mapViewer.checkLeftObstacles(robotSimulator)){
            if (Main.isSimulating())
                System.out.println("Send calibration command " + SensorCalibration.Left.toString());
            else
                Main.getRpi().sendCalibrationCommand(SensorCalibration.Left);
        }
        
        // only front back on the right side
        if (mapViewer.checkRightFrontBack(robotSimulator)){
            if (Main.isSimulating())
                   System.out.println("Send calibration command " + SensorCalibration.Right.toString());
            else
                Main.getRpi().sendCalibrationCommand(SensorCalibration.Right);
        }
    }
}
